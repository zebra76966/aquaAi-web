        ...
        <div key={i} className={`ug-item${open === i ? " open" : ""}`}>
          ...
          {open === i && (
            <div className="ug-answer">
              <p>{s.body}</p>
              <a href="#" className="ug-video-link"><FiExternalLink /> Watch video walkthrough (coming soon)</a>
            </div>
          )}
        </div>
      ))}
    ...
    <section className="ug-section" id="user-guides">
      <div className="ug-inner">
        <div className="ug-heading">
          ...
          <h2 className="ug-title">Everything you need to <span className="ug-accent">get started</span></h2>
        </div>

        {/* Main tab */}
        <div className="ug-tabs">
          <button className={`ug-tab${tab === "users" ? " active" : ""}`} onClick={() => setTab("users")}>Users App</button>
          <button className={`ug-tab${tab === "providers" ? " active" : ""}`} onClick={() => setTab("providers")}>Providers App</button>
        </div>

        {tab === "users" && (
          <div className="ug-panel">
            <div className="ug-download-bar">
              <a href="#" className="ug-download-btn"><FiDownload /> Download Full User Guide (PDF)</a>
            </div>
          ...
          <div className="ug-panel">
            <div className="ug-provider-tabs">
              {Object.keys(PROVIDER_SECTIONS).map((k) => (
                <button key={k} className={`ug-provider-tab${providerTab === k ? " active" : ""}`} onClick={() => setProviderTab(k)}>{k}</button>
              ))}
            </div>
            <div className="ug-download-bar">
              <a href="#" className="ug-download-btn"><FiDownload /> Download {providerTab} Guide (PDF)</a>
            </div>
